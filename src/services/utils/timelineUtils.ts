...
export const extractAllHistoricalEvents = (cvrData: any, financialData?: any): TimelineEvent[] => {
  ...
  // Extract additional historical data from _enrichedDeltagerData
  if (normalizedData?.deltagerRelation) {
    normalizedData.deltagerRelation.forEach((relation: any) => {
      const personName = relation.deltager?.navne?.[0]?.navn || 
                        relation._enrichedDeltagerData?.navne?.[0]?.navn || 
                        'Ukendt person';
      
      // Process virksomhedSummariskRelation for historical company associations
      const enrichedRelations = relation._enrichedDeltagerData?.virksomhedSummariskRelation || [];
      
      enrichedRelations.forEach((virkRelation: any) => {
        const companyName = virkRelation.virksomhed?.navne?.[0]?.navn || 'Ukendt virksomhed';
        const companyCvr = virkRelation.virksomhed?.cvrNummer;
        
        virkRelation.organisationer?.forEach((org: any) => {
          if (org.medlemsData) {
            org.medlemsData.forEach((medlem: any) => {
              const startDate = parseDate(medlem.attributter?.find((a: any) => a.type === 'FUNKTION')?.vaerdier?.[0]?.periode?.gyldigFra || medlem.periode?.gyldigFra);
              const endDate = parseDate(medlem.attributter?.find((a: any) => a.type === 'FUNKTION')?.vaerdier?.[0]?.periode?.gyldigTil || medlem.periode?.gyldigTil);
              ...
              if (startDate) {
                ...
                if (org.hovedtype === 'LEDELSESORGAN' || org.hovedtype === 'REGISTER_DIREKTION') {
                  category = 'management';
                  severity = 'high';
                } else if (org.hovedtype === 'REGISTER_BESTYRELSE') {
                  category = 'board';
                ...
                const orgName = org.organisationsNavn?.[0]?.navn || org.hovedtype;
                const description = companyCvr 
                  ? `${roleTitle} - ${orgName} (CVR: ${companyCvr})`
                  : `${roleTitle} - ${orgName}`;
                ...
                events.push({
                  ...
                  newValue: personName,
                  severity,
                  metadata: { relation, virkRelation, org, medlem, type: 'entry', companyName, companyCvr },
                });
                ...
                if (endDate) {
                  events.push({
                    ...
                    oldValue: personName,
                    severity: 'medium',
                    metadata: { relation, virkRelation, org, medlem, type: 'exit', companyName, companyCvr },
                  });
                }
              }
            });
